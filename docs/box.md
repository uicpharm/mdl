...
::: info This has already been done for our Pharmacy Box account
The following steps should be followed if new integration needs to be set up with a new
Box account. This only has to be done once for each Box account. So, there is no harm
in reproducing these steps, but you can also forgo these steps and share credentials.
:::
...
5. Set the redirect URI to: <https://docs.uicpharm.dev/auth/callback/>
...
The authorization saves two files in your environment: `access_token.txt` and
`refresh_token.txt`. These tokens will be regularly refreshed as needed when they expire.
